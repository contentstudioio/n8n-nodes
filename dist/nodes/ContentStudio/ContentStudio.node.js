                    ...
                    options: [
                        {
                            ...
                            description: 'Returns the plan summary, the 14 metered entitlements and the monthly usage-reset window, built live on every request. The request-rate ceiling is not part of the body — read it from the X-RateLimit-Limit / X-RateLimit-Remaining response headers.',
                        },
                    ],
                    default: 'get',
                },
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'post' && (operation === 'create' || operation === 'update')) {
                    ...
                    const enableRepeat = canRepeat && this.getNodeParameter('enableRepeat', i, false);
                    if (enableRepeat) {
                        const repeatType = this.getNodeParameter('repeatType', i, 'Week') || 'Week';
                ...
                if (resource === 'contentCategory' && operation === 'update') {
                    ...
                    if (allowedMemberIds.length)
                        body.allowed_member_ids = allowedMemberIds;
                    if (categoryAccounts.length)
                        body.accounts = categoryAccounts;
                    options.method = 'PUT';
                    options.url = `${baseRoot}/v1/workspaces/${workspaceId}/content-categories/${categoryId}`;
                    options.body = body;
                }
                ...
                if (resource === 'shareLink' && operation === 'list') {
                    ...
                    const page = this.getNodeParameter('page', i);
                    const perPage = this.getNodeParameter('perPage', i);
                    const search = (this.getNodeParameter('shareLinkSearch', i, '') || '').trim();
                    options.method = 'GET';
                ...
                if (resource === 'shareLink' && operation === 'create') {
                    ...
                    const notes = (0, utils_1.parseCommaSeparated)(this.getNodeParameter('shareLinkNotes', i, ''));
                    const isPasswordProtected = this.getNodeParameter('shareLinkIsPasswordProtected', i, false);
                    ...
                    options.body = {
                        ...
                        ...(plans.length ? { plans } : {}),
                        ...(notes.length ? { notes } : {}),
                        show_notes: this.getNodeParameter('shareLinkShowNotes', i, false),
                        is_password_protected: isPasswordProtected,
                        ...(isPasswordProtected ? { password } : {}),
                        is_single_post: isSinglePost,
                        allow_external_comments: this.getNodeParameter('shareLinkAllowExternalComments', i, false),
                ...
                if (resource === 'shareLink' && operation === 'update') {
                    ...
                    if (fields.notes !== undefined && fields.notes !== null && fields.notes !== '') {
                        body.notes = (0, utils_1.parseCommaSeparated)(fields.notes);
                    }
